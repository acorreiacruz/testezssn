...
class TestAPIZssn(APITestCase, TesteBaseMixin):
    ...
    def get_resposta(self, action="list", method="get", data=None, **kwargs):
        ...
        if method == "post":
            resposta = self.client.post(
                self.get_api_url(action=action, **kwargs), data=data
            )
    ...
    def test_se_api_atualiza_o_sobrevivente(self):
        local = {"latitude": -13.2719, "longitude": -48.2852}
        sobrevivente = self.criar_sobrevivente(**local)

        resposta1 = self.get_resposta(
            "detail",
            "patch",
            {"latitude": 76.6762, "longitude": -82.2616},
            pk=sobrevivente.id,
        )
        ...
        self.assertEqual(
            resposta1.data.get("latitude"),
            resposta2.data.get("latitude")
        )
        self.assertEqual(
            resposta1.data.get("longitude"), resposta2.data.get("longitude")
        )

    def test_se_api_retorna_status_code_404_ao_tentar_atualizar_inventario(
        self
    ):
        dados_inventario = {
            "agua": 23,  # 5 pontos
    ...
    def test_se_api_retorna_status_code_404_ao_atualizar(self):
        resposta = self.get_resposta("detail", "patch", pk=10)
        self.assertEqual(resposta.status_code, 404)
    ...
    def test_se_api_retorna_relatorio_de_medias_dos_inventarios(self):
        ...
        self.criar_conjunto_de_sobreviventes(25)
